...
def explorer(launcher):
    ...
    launcher.bind_(solver="musicgen/musiversal_32khz")
    # replace this by the desired music dataset, which needs to be stereo
    launcher.bind_(dset="musiversal")
    ...
    launcher.slurm_(gpus=1).bind_(label="gpu")
    with launcher.job_array():
        sub = launcher.bind(
            {"continue_from": str(checkpoints / "stereo_finetune_musicgen-small.th")}
        )
        sub(small, cfg_low, wd_low, adam, {"optim.max_norm": 3})

    launcher.slurm_(gpus=2).bind_(label="2gpus")
    with launcher.job_array():
        sub = launcher.bind(
            {"continue_from": str(checkpoints / "stereo_finetune_musicgen-small.th")}
        )
    ...
    with launcher.job_array():
        sub = launcher.bind(
            {"continue_from": str(checkpoints / "stereo_finetune_musicgen-medium.th")}
        )
        sub(medium, adam)
    ...
    with launcher.job_array():
        sub = launcher.bind(
            {"continue_from": str(checkpoints / "stereo_finetune_musicgen-large.th")}
        )
